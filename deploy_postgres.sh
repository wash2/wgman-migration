...
docker run -d --restart always --name "$WGMAN_DB_CONTAINER_NAME" -e POSTGRES_USER="$WGMAN_DB_USER" -e POSTGRES_PASSWORD="$WGMAN_DB_PW" -e POSTGRES_DB="$WGMAN_DB_NAME" -p $WGMAN_DB_PORT postgres
